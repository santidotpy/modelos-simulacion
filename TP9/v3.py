...
pygame.display.setCaption("Simulación de Depósito de Partículas")
...
while corriendo:
    ...
    if not crecimiento_alcanzado:
        ...
        while not adherida:
            ...
            colision = False
            for p in particulas:
                if nueva_particula.colisiona_con(p):
                    nueva_particula.adherida = True
                    particulas.append(nueva_particula)
                    adherida = True
                    break
            if nueva_particula.colisiona_con_pared(ancho_conducto, alto_conducto):
                nueva_particula.adherida = True
                particulas.append(nueva_particula)
                adherida = True
